...
bnd = bnd_L;
n = mode_in;
[P_inc,T_inc,Y] = nodal_displacement_and_force( wd,kd,n,Amp,bnd,Coordinate,lambda,mu,density,h,0 );
...
for n = 1:modes	
	[P_Ln,T_Ln,Y] = nodal_displacement_and_force( wd,kd,n,Amp,bnd,Coordinate,lambda,mu,density,h,0 );
	P_L(:,n) = P_Ln;
	T_L(:,n) = T_Ln;
end
...
for n = 1:modes
	[P_Rn,T_Rn] = nodal_displacement_and_force_R( wd,kd,n,Amp,bnd,Coordinate,lambda,mu,density,h,-pi/2,Y );
	P_R(:,n) = P_Rn;
	T_R(:,n) = T_Rn;
end
...
% figure
% subplot(2,1,1)
% plot(linspace(0,10e-3,length(bnd_T)),u_Tx,'b')
% title('u')
% subplot(2,1,2)
% plot(linspace(0,10e-3,length(bnd_T)),u_Ty,'r')
% title('v')
