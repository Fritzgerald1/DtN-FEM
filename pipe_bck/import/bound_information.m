function [left, rigth, top ,bottom, freeBnd, left_e, right_e, freeBnd_e] = bound_information(Coordinate, Ielement)
x = Coordinate(:,1);
y = Coordinate(:,2);
L=15*10^(-3);
W=1*10^(-3);
...
[A,~] = arrayfun(@(x) find(ismember(Ielement,x)),rigth(:),'UniformOutput',false);
